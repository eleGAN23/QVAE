...
parser.add_argument('--n_channels', type=int, default=4, help="3 for real-valued inputs, 4 for quaternion-valued inputs.")
...
def loss_function(recons, input, mu, log_var, kld_weight=opt.kld_weight) -> dict:
...
##### MODEL SETUP #####
qvae = QuaternionVanillaVAE(in_channels=opt.n_channels, latent_dim=opt.latent_dim)
print("Number of parameters", sum(p.numel() for p in qvae.parameters() if p.requires_grad))
if opt.cuda:
    qvae.cuda()
...
def train(model):
    ...
    for epoch in range(opt.n_epochs):
        ...
        scheduler.step()
        # Store losses
        with open('checkpoints/loss_train_midqvae_linearlayers_epoch%i' %epoch, 'wb') as fp:
            pickle.dump(loss_train, fp)
        with open('checkpoints/loss_val_midqvae_linearlayers_epoch%i' %epoch, 'wb') as f:
            pickle.dump(loss_val, f)
        # Store model
        torch.save(model.state_dict(), "checkpoints/model_midqvae_newloss_epoch%i" %epoch)
